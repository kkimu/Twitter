...
sample = ["bfs","dfs","sec"]
  sample.each do |method|
    for i in 1..N
      ...
      cent.each do |j|
        ...
        per.each do |k|
          count = 0
          sum = 0
          open("work/result/#{i}/#{method}_#{j}_#{k}per.txt").each do |line|
            if count < n
              a = line.split(" ")
        ...
        open("result/#{i}/#{method}_mf#{n}_#{j}.txt","w") do |f|
          f.puts result
        end
      end
    end
    ...
    cent.each do |i|
      ...
      for j in 1..N
        k=0
        open("result/#{j}/#{method}_mf#{n}_#{i}.txt").each do |line|
          ave[k] += line.to_f
          k += 1
        end
      end
      ...
      open("result/result/#{method}_mf#{n}_#{i}.txt","w") do |f|
        f.puts ave
      end
    end
  end
